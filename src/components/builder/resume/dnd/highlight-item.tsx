...
interface ListHighlightItemProps<T extends { id: string; value: string }> {
	item: T
	getItemData(
		item: T,
...
export function ListHighlightItem<T extends { id: string; value: string }>({
	item,
	getItemData,
	isItemData,
	onDelete,
...
function DragPreview<T extends { id: string; value: string }>({
	item,
}: {
	item: T
}) {
		...
		<Card className="w-full">
			<CardContent className="p-4 flex items-cetner justify-between">
				...
				<div className="flex space-x-2">
					<TooltipWrapper tooltip={m['highlight-item.delete']()}>
