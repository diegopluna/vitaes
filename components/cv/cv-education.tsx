                ...
                {props.educations.map((education, index) => (
                    <div className="w-full" key={index}>
                        <div className="flex justify-between w-full">
                            ...
                            <span className="text-xs italic text-[#0395de]">{education.location}</span>
                        </div>
                        <div className="flex justify-between w-full">
                            <span className="text-xs text-gray-600">{education.degree.toUpperCase()}</span>
                            <span className="text-xs text-gray-500 italic ">{education.startDate}{' - '}{education.endDate}</span>
                        </div>
                        <div className="flex ml-4">
                            <ul className="flex flex-col list-disc">
                                {education.description.map((description, descriptionIndex) => (
                                    <li key={descriptionIndex} className="text-xs text-gray-500">{description}</li>                             
                                ))}
                            </ul>
                        </div>
                    </div>
